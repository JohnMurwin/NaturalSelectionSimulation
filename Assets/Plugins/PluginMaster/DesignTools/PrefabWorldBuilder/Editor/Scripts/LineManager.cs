...
namespace PluginMaster
{
    ...
    public class ObjectPose
    {
        ...
        public ObjectPose(ObjectId id, Vector3 position, Quaternion localRotation, Vector3 localScale)
            => (_id, _position, _localRotation, _localScale) = (id, position, localRotation, localScale);

        public ObjectId id { get => _id; set => _id = value; }
       
        public GameObject obj => ObjectId.FindObject(_id);
    ...
    public class PersistentLineData
    {
        ...
        public void UpdateObjects()
        {
            ...
            {
                var obj = ObjectId.FindObject(item.id);
    ...
    public class SceneLineData
    {
        ...
        public SceneLineData(string sceneGUID) => _sceneGUID = sceneGUID;
        ...
        public void RemoveLineData(long lineId) => _lines.RemoveAll(l => l.id == lineId);
    ...
    public static partial class PWBIO
    {
        ...
        private static void LineInitializeOnLoad() => LineManager.settings.OnToolModeChanged += OnLineToolModeChanged;
        ...
        private static void LineDuringSceneGUI(SceneView sceneView)
        {
            ...
            {
                LineManager.settings.editMode = !LineManager.settings.editMode;
                ToolProperties.RepainWindow();
            }*/
            if (LineManager.settings.editMode)
            {
                LineToolEditMode(sceneView.camera);
                return;
            }
        ...
        private static bool DrawLineControlPoints(LineData lineData, out bool clickOnPoint, out bool wasEdited)
        {
            clickOnPoint = false;
            wasEdited = false;
            ...
            if (lineData.selectedPointIdx >= 0)
            {
                ...
                var point = SnapAndUpdateGridOrigin(lineData.selectedPoint, SnapManager.settings.snappingEnabled,
                ...
                if (prevPosition != lineData.selectedPoint) wasEdited = true;
            }
            return clickOnPoint || wasEdited;
        }

        private static void LineToolEditMode(Camera SceneCamera)
        {
            ...
            foreach (var lineData in persistentLines)
            {
                ...
                if (DrawLineControlPoints(lineData.data, out bool clickOnPoint, out bool wasEdited))
                {
                    if (clickOnPoint)
                    {
                        _edittingPersistentLine = true;
                        if (selectedLineId != lineData.id) ApplySelectedPersistentLine(false);
                ...
                if (_edittingPersistentLine)
                    LineStrokePreview(SceneCamera, lineData.settings, lineData.data.lastPathPoint);
            }
        }
        ...
        private static void ApplySelectedPersistentLine(bool deselectPoint)
        {
            ...
            {
                var objs = Paint(LineManager.settings);
        ...
        private static void LineStateBezier(Camera camera)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                LineStrokePreview(camera, LineManager.settings, _lineData.lastPathPoint);
                DrawLine(_lineData);
                if (_selectingLinePoints)
                {
                    var rays = new Ray[]
            ...
            DrawLineControlPoints(_lineData, out clickOnPoint, out wasEdited);
            bool leftMouseDown = Event.current.button == 0 && Event.current.type == EventType.MouseDown;
            ...
            for (int i = 0; i < BrushstrokeManager.brushstroke.Length; ++i)
            {
                ...
                if (settings.objectsOrientedAlongTheLine && BrushstrokeManager.brushstroke.Length > 1)
                {
                    ...
                    segmentDir = i < BrushstrokeManager.brushstroke.Length - 1
                        ? strokeItem.nextTangentPosition
                        - strokeItem.tangentPosition

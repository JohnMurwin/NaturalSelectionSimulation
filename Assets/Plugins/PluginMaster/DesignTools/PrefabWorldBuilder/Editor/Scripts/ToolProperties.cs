    ...
    public class ToolProperties : EditorWindow
    {
        ...
        private void ModifierGroup(IModifierTool settings)
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var onlyTheClosest = EditorGUILayout.ToggleLeft(actionLabel + " only the closest",
        ...
        private void LineGroup()
        {
            ...
            /*using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    var editMode = EditorGUILayout.ToggleLeft("Edit Mode", LineManager.settings.editMode);
                    if(check.changed)
                    {
                        Undo.RegisterCompleteObjectUndo(this, UNDO_MSG);
                        LineManager.settings.editMode = editMode;
                        SceneView.RepaintAll();
                    }
                }
            }*/

...
namespace PluginMaster
{
    ...
    public static class BrushstrokeManager
    {
        ...
        private static float GetLineSpacing(Transform transform, LineSettings settings)
        {
            ...
            {
                var bounds = BoundsUtils.GetBoundsRecursive(transform, transform.rotation);
        ...
        {
            _brushstroke.Clear();
            var objPositionsList = new List<Vector3>();
            var strokePositionsList = new List<Vector3>();
            float lineLength = 0f;
            ...
            do
            {
                ...
                float spacing = objectExist ? GetLineSpacing(lineObjects[objIdx].transform, settings)
                    : GetLineSpacing(nextIdx, settings);
                length += Mathf.Max(spacing, minSpace);
                if (length > lineLength) break;
                if (objectExist)
                {

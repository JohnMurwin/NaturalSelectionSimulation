						...
						foreach (var _position in tileMap.clusters[modifiedClusters[c]].Keys)
						{
							...
							if (instantiateChilds)
							{
								...
								for (int i = 0; i < count; i ++)
								{
									...
									var _childObj = MonoBehaviour.Instantiate(childPrefab, _rndPos, _tileObject.transform.localRotation);
					...
					foreach (var _position in tileMap.clusters[_cluster].Keys)
					{
						...
						if (instantiateChilds)
						{
							...
							for (int i = 0; i < count; i ++)
							{
								...
								var _childObj = MonoBehaviour.Instantiate(childPrefab, _rndPos, Quaternion.identity);
